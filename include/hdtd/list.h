 ...
 */
_INLINE_ void __list_add(void * add, void * prev, void * next)
{
...
#define dmlist_add_tail(add, head) __list_add(add, (head)->prev, head);
...
#define dmlist_del(entry) list_del((struct list_head *)(entry))
...
#define dmlist_empty(head) list_empty((struct list_head *)(head))
...
#define list_last(head) \
	((head) && (head)->prev != (head) ? (head)->prev : NULL)
...
_INLINE_ struct list_head * list_at(struct list_head *head, int index)
{
    struct list_head *p;
    for (p = head->next; index != 0; p = p->next, index--) {
        if(p == head)
            return NULL;
    }
    return p;
}
#define dmlist_at(head, index) list_at((struct list_head *)head, index)
...
#define dmlist_for_each(pos, head) list_for_each(pos, head)
